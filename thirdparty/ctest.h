/* Copyright 2011-2021 Bas van den Berg1
 *
...
void assert_str(
        const char* exp, const char* real, const char* caller, int line);
...
void assert_not_equal(
        intmax_t exp, intmax_t real, const char* caller, int line);
...
void assert_equal_u(uintmax_t exp, uintmax_t real, const char* caller, int line)
{
    if (exp != real) {
        CTEST_ERR(
                "%s:%d  expected %" PRIuMAX ", got %" PRIuMAX,
                caller,
                line,
                exp,
                real);
